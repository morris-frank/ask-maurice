...
src/ask_maurice/
  ...
  config.py           BuildConfig (private vault) and RuntimeConfig (shared only) — the
                      ...
                      SlackConfig is the only access edge, and production
                      requires it
  ...
  runtime/
    ...
    identity.py         Slack user ID (or an `--as` handle) -> email -> Participant,
                        via the alias table baked into the bundle. The email is the
                        join key, not any provider's id, which is why removing two
                        edges left this module almost untouched. Opens no vault file.
    ...
    server.py           FastAPI app. /slack/command is the only authenticated route:
                        ...
                        edge means the verifier, the RFC 9728 route and PyJWT come
                        back together, and re-adding IAP is not small at all — it
                        fronts the whole service and would intercept /slack/command
scripts/
